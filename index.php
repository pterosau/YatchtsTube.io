...
nav {
  z-index: 99; /* Ensure the navigation bar is above other content */
...
<script src="script.js"></script>
  <!-- Form for uploading new video URL -->
  <form id="uploadForm">
    <input type="text" id="videoUrlInput" placeholder="Enter YouTube URL">
    <button type="submit">Upload</button>
  </form>

  <!-- JavaScript to handle form submission and dynamic content -->
  <script>
    // JavaScript code for handling form submission and dynamic content
    $(document).ready(function() {
      // Function to add a new YouTube video URL to the list
      $('#uploadForm').submit(function(event) {
        event.preventDefault();
        var videoUrl = $('#videoUrlInput').val().trim();
        if (videoUrl !== '') {
          ...
          if (isValidYouTubeUrl(videoUrl)) {
            ...
            // Create new list item with thumbnail and link
            var listItem = '<li>' +
                             '<div class="thumbwrap">' +
                               '<img src="' + thumbnailUrl + '" />' +
                               '<div class="panel"><div class="panelinner"><i class="play fa fa-play-circle-o"></i> <a class="externallink fa fa-external-link-square" href="' + videoUrl + '" target="_blank"></a></div></div>' +
                             '</div>' +
                             '<a href="' + videoUrl + '" target="_blank">' + videoUrl + '</a> <button class="deleteButton">Delete</button>' +
                           '</li>';
            $('#youtubelist').append(listItem);
            $('#videoUrlInput').val(''); // Clear input field
          } else {
    ...
    // JavaScript function to extract YouTube video ID from URL
    function getyoutubeid(link) {
      var youtubeidreg = /(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/ ]{11})/i;
      return youtubeidreg.exec(link)[1];
    }

    // JavaScript function to validate YouTube URL
